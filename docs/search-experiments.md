...
Of 240 pairs, 222 had no evidence difference. Policy replay gained authored evidence in two low-budget storylet pairs, but lost seven deep-chain knots in two 100-state/depth-300 pairs. Four apparent critical losses and gains were the same content-exhaustion error receiving different approximate source-line identities; #84 later corrected the identity and evaluator contract without rewriting this historical result. All 80 largest-budget pairs were neutral except the random/turn family, where the Ink story RNG is not controlled by the search seed and both strategies failed repeat equality in 10 of 12 matrix points.
...
The full 240-pair matrix was rerun. All 228 non-random pairs were evidence-identical across critical findings, assertions, authored knots/outcomes, terminal identities, proof, and limits. The 12 remaining symmetric authored differences came from uncontrolled Ink `RANDOM()` sequences, motivating #117. This resolved #118 without promoting policy v2: corrected parity removed false behavior but did not demonstrate a dynamic-allocation gain.

### Explicit Ink runtime seed (#117)

Inkcheck now initializes Ink runtime randomness from a separate `storySeed` (default 1) while preserving the existing search-sampling seed. Same-source fresh processes at story seed 17 reproduce the same semantic-feature ending; story seed 18 produces a different valid random outcome while the pass allocation remains unchanged. The full promotion matrix, rerun with the random/turn family explicitly pinned to story seed 1, is now evidence-identical in all 240 pairs with zero repeat failures. This makes matched strategy comparisons and witness replay credible under a declared random sequence; it does not enumerate or prove behavior across every story seed.
