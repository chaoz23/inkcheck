...
An explicitly concurrent portfolio adds `explore.execution`: requested/effective concurrency plus canonical per-pass grants, consumption, and completion/resource/failure status. Persistent worker slots retain pass state across the production adaptive rounds; pass grants and final merge order remain deterministic regardless of worker completion timing. A failed worker sets `truncatedBy.worker` and `bindingLimit: "worker"` unless another systematic pass independently proves exhaustion; the latest completed snapshots remain valid partial evidence. Concurrency defaults to one until the broad promotion and resource/cancellation gates pass.
