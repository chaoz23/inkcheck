...
`shadowDecision` is Inkcheck's versioned, deterministic v0.6 decision-policy hypothesis over those facts. It is deliberately advisory: `mode` is `shadow`, `applied` is always `false`, and it cannot change allocation or stop a run. The action vocabulary is `continue`, `reallocate`, `probe`, `stop_at_knee`, `stop_at_deadline`, `stop_at_resource_limit`, and `stop_exhaustive`. Each decision records the binding constraint, uncertainty, evidence, reason, and current/suggested per-pass shares. Suggested shares preserve an 8% fractional probe floor (or an equal share for portfolios with more than 12 passes).

The research-only `explorePortfolioShadowReplay` result adds `explore.policyReplay[].floorService`. It records the planned integer grants and floor-only grants for that window plus each pass's cumulative promise, floor service, released service, total grant, debt, and credit. Fractional promises pool across windows; deterministic largest-debt service prevents a small window from repeatedly rounding one active pass to zero. A pass that completes receives no new promise and releases any outstanding fractional debt. This diagnostic is not emitted by ordinary production portfolio runs, whose allocator remains unchanged.
