...
`shadowDecision` is Inkcheck's versioned, deterministic v0.6 decision-policy hypothesis over those facts. It is deliberately advisory: `mode` is `shadow`, `applied` is always `false`, and it cannot change allocation or stop a run. The action vocabulary is `continue`, `reallocate`, `probe`, `stop_at_knee`, `stop_at_deadline`, `stop_at_resource_limit`, and `stop_exhaustive`. Each decision records the binding constraint, uncertainty, evidence, reason, and current/suggested per-pass shares. Suggested shares preserve an 8% probe floor (or an equal share for portfolios with more than 12 passes) so a temporarily quiet strategy is never silently starved.
