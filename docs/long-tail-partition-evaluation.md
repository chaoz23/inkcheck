...
The 0.6 promotion gate is separately predeclared in `benchmarks/long-tail-promotion-v2.json`: Dog Ink Adventure, The Intercept, and Heresy II each receive matched 5M-state same-frontier and independent-partition arms at depth 100. Every campaign has a 14-minute graceful deadline inside a 15-minute isolated-worker deadline. The worker atomically saves the base and every completed result window. A hard timeout after a snapshot is partial evidence; a timeout before the first snapshot is explicitly unavailable. Neither is treated as parity, failure to find defects, or evidence that a story is covered. The ordinary internal deadline remains the cross-platform worker-tree shutdown path. A fallback hard timeout kills the isolated process group on Unix and the exact coordinator process on Windows; machine output names that scope instead of overstating cleanup.
...
# Full 0.6 promotion gate; writes each arm/window atomically as it completes.
node --max-old-space-size=6144 dist/long-tail-evaluation-cli.js \
  benchmarks/long-tail-promotion-v2.json \
  --output benchmarks/results/long-tail-promotion-v2.json
```
