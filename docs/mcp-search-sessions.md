...
`start_campaign` defaults to a `balanced` named mode. `quick`, `deep`, `overnight`, and `campaign` provide progressively larger bounded postures; `fixed` and the deprecated `intent` input preserve explicit callers. Optional state/window/time/memory/disk/deadline ceilings, resource/value/stop preferences, long-tail share/probe count, regression reserve, seeds, depth, and frontier envelopes remain bounded and replayable. Concurrency is fixed at one.
