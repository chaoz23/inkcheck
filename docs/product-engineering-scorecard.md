...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **8/10** | The evaluation fixed an oversized-checkpoint `Invalid string length` crash, added worker headroom so 5M bases retain memory-stopped reports, and reduced the measured 500K Intercept checkpoint from above 512 MiB to 54.1 MiB without raising its safety ceiling. Hosted persistence, global concurrency, and externally reconciled cost controls remain (#40, #93, #94, #152, #156). |
