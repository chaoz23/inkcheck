...
Updated 2026-07-13 against the Inkcheck 0.5.1 release plus unreleased 0.6 research on `main`, the complete synthetic policy-v2 matrix, and the first authored-project promotion tier.
...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **5/10** | Policy v2 adds window-normalized marginal yield, bounded renewal/decay, warm-up, priority gates, and truthful baseline parity while gated off. The authored tier exposes severe shape-dependent throughput: Dog Ink Adventure could not complete a 1M baseline inside five minutes, while The Intercept completed matched 5M runs in about 11 minutes each. Dynamic stopping remains a product need, not a proven policy win. |
...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **7/10** | Hard state/depth ceilings, shared CLI/MCP/evaluation guards, bounded curves, atomic evaluation snapshots, hosted limits, and partial reports exist. Shared search now releases dead checkpoint/ancestry payload, compacts stale views, exposes component accounting, and supports explicit count/byte envelopes. Disk spill, global concurrency accounting, persistence, and campaign ceilings remain (#40, #93, #94, #98). |
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **4/10** | Base shared search has exact split-run equivalence plus private, atomic, source-bound CLI checkpoints; completed reports add bounded storage, stable finding drill-down, exact replay, and explicit cleanup (#63, #128, #132, #134, #136). Assertion/goal checkpoints, MCP/hosted campaigns, migration, and specialist partitioning remain (#66, #86, #98). |
