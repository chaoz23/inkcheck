...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **6/10** | Durable MCP campaigns now execute exact policy-bound windows across fresh processes, expose marginal yield/spend/provenance, enforce aggregate deadlines/resources between windows, and retain the latest partial report. They remain one shared trajectory with fixed concurrency; policy v2 is still gated off, and multi-run allocation, proven dynamic stopping, and human UX remain (#93-#96). |
| **Author and agent intent** | Humans and agents can safely declare invariants, goals, deadlines, and resource posture; Inkcheck verifies them mechanically and explains what it could not establish. | **7/10** | Typed assertions, additive and staged goals, durable MCP sessions, exact replay, regression pins, and root-started additive goal probes exist with separate protected-base accounting. Goal reprioritization, causal goal quality, deadline/resource posture, and the hosted assistant remain (#66, #81, #86, #87, #95, #96). |
