...
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **4/10** | Base shared search now has exact split-run equivalence plus private, atomic, source-bound CLI persistence with quotas and retention (#130, #132). Assertions/goals, MCP/hosted campaigns, migration, and specialist partitioning remain (#63, #86, #98). |
