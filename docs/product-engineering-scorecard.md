...
| --- | --- | ---: | --- |
| **Actionable, repeatable QA** | Mechanical defects become stable findings with exact replay; fixes can be checked again in CLI, CI, web, or MCP. | **8/10** | Compile/runtime findings, assertions, indexed witnesses, stable private reports, exact replay, and runtime regression pins now support a durable MCP search-fix-recheck loop. Approximate runtime mappings no longer change finding IDs across strategies or witness shortening. Assertion/ending pins and broader post-edit campaign proof remain (#66). |
