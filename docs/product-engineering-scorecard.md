...
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **3/10** | Base shared search has a source-bound schema-v1 engine checkpoint with exact split-run equivalence (#130). Durable CLI persistence, retention policy, assertions/goals, specialist partitioning, and resumable campaigns remain (#63, #86, #98). |
