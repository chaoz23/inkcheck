...
Updated 2026-07-15 after compact durable checkpoints and stable approximate-runtime identity. The fixed portfolio remains the default and policy v2 remains shadow-only.
...
| --- | --- | ---: | --- |
| **Actionable, repeatable QA** | Mechanical defects become stable findings with exact replay; fixes can be checked again in CLI, CI, web, or MCP. | **8/10** | Compile/runtime findings, assertions, indexed witnesses, stable private reports, exact replay, and private evidence pins for runtime, assertion, and approved-goal witnesses now support a durable MCP search-fix-recheck loop. The default five-tool MCP profile plus bundled skill measures about 2,738 bootstrap tokens under the preregistered byte estimate; the two-implementation agent gate and broad post-edit campaign proof remain unproven (#67, #66). |
...
| **Robust unknown-shape exploration** | The default gives strong broad insurance across early choices, deep suffixes, variables, loops, gates, storylets, randomness, and sparse failures without one family dominating. | **6/10** | DFS, beam, seeded random, shared-frontier experiments, and adaptive rounds are materially stronger than DFS alone. Workload-aware concurrency now executes the same grants without changing findings or proof, but completed authored-project cells still show no broad allocation-policy advantage. Specialist breadth and public-corpus evidence remain limited. |
...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **7/10** | Durable campaigns execute exact base windows plus protected independent partitions. The 0.6 gate reproduced strong Intercept diversity/memory gains, but Dog spent 14 minutes and Heresy hit memory before either 500K base became resumable. Fixed first-window sizing can consume the whole UX envelope before policy acts; dynamic first-window sizing is now the principal gap. |
| **Author and agent intent** | Humans and agents can safely declare invariants, goals, deadlines, and resource posture; Inkcheck verifies them mechanically and explains what it could not establish. | **8/10** | Typed assertions and staged goals can become explicit campaign children alongside exact replay, regression pins, and named resource/value/stop posture. A versioned bundled skill, ten golden exercises, and an executable no-hidden-hints scorer now define the safe workflow; the required two distinct observational agent runs remain unproven (#67), and automatic goal quality, assertion/ending pins, and the hosted assistant remain (#66, #81, #86, #87). |
...
| **Demonstrated generalization** | Claims are supported across at least 20 consent-safe public stories or structural families, multiple budgets/depths/seeds, and medium/large projects as well as small-source/large-state stories. | **4/10** | The executable 20-family synthetic matrix has a pinned public tier: Dog Ink Adventure, The Intercept, and Heresy II. The 0.6 gate retained honest results for all three under 14-minute arms, but only The Intercept reached long-tail allocation; Dog was time-bound and Heresy memory-bound in the base. This is enough to reject universal promotion, not enough to claim broad advantage. |

## Engineering truths

| Engineering truth | 10/10 target | Current | Evidence and principal gap |
| --- | --- | ---: | --- |
| **Deterministic evidence and replay** | Fixed source/config/seed/policy/event log reproduces decisions, allocations, finding identities, and witness paths across supported platforms. | **8/10** | Fixed-seed engines, indexed witnesses, versioned schemas, decision logs, stable approximate-runtime IDs, and cross-platform CI are strong. Broader resumable event-log replay still needs work (#63, #66). |
| **Critical evidence cannot be averaged away** | Runtime errors and assertion violations remain separate, identity-based promotion gates; no aggregate score or terminal count can hide their loss. | **8/10** | Report/evaluator tiers and paired comparisons enforce the principle, #105 removes the first semantic critical loss, and benchmark comparisons now separate semantic runtime retention from mutable location metadata. The corpus remains too narrow for a higher score (#56, #103). |
| **Portfolio-marginal value accounting** | Explorers and specialists earn discretionary work only for portfolio-new evidence or explicit intent progress; duplicate rediscovery is visible but not double-paid. | **8/10** | Campaign children deduplicate canonical evidence across prior reports, while independent-child ledgers preserve observed-versus-campaign-new counts and discovery spacing. The Intercept gate measured terminal rediscovery rising from 34% to 94% while marginal yield declined but stayed nonzero. That evidence supports inspectable rotation, but not a live stop; the promotion gate closed without changing allocation. |
...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **8/10** | Independent Intercept children held peak campaign heap to 1.41 GiB and completed 5M states while a growing shared frontier stopped at 3.70 GiB. Aggregate state/time/memory/disk accounting and compressed-checkpoint bytes remained explicit. A 600K preflight exposed a larger-checkpoint readback boundary, and externally reconciled cost plus broader storage adversaries remain (#152, #156). |
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **6/10** | Exact split-run equivalence remains strong, and nine independent Intercept children preserved the qualified 500K base checkpoint byte-for-byte while avoiding its frontier-growth trap. Children still restart from the root rather than sharing frontier work; a 600K decode preflight failed, and frontier reuse, reprioritization, hosted campaigns, adversarial storage accounting, and a second public family remain (#66, #86, #156). |
| **Inspectable decisions** | Every automatic activation, allocation, forecast, contraction, and stop records policy version, factual inputs, uncertainty, reason, and binding constraint in compact human/agent forms. | **9/10** | Compact MCP output includes concurrency activation evidence plus a versioned shadow-only long-tail recommendation with value preference, floor, authorization, rates, trend, selected-value rediscovery, report-local gaps, uncertainty, and unavailable signals. `liveEffect: false` remains evidence-backed after the three-family no-promotion gate. Externally accounted cost decisions remain absent (#152). |

## Proposed 0.7 assessment gate

Inkcheck 0.7 ships the trustworthy author-defined rule foundation: the hosted checker can submit one explicit numeric rule into ordinary bounded QA, while local config, CI, and MCP retain the full typed grammar. This improves access to existing assertion evidence but does not change a score by itself. The experimental specialist target remains to raise structured specialist advantage from 4/10 to 7/10 by proving that author-declared invariants can receive bounded additive search without weakening baseline QA. The related product-truth targets are actionable repeatable QA 8 -> 9, honest bounded evidence 8 -> 9, unknown-shape exploration 6 -> 7, anytime value 7 -> 8, author and agent intent 8 -> 9, human trust 7 -> 8, and demonstrated generalization 4 -> 6.

Those target movements are specialist-promotion gates, not forecasts. The shipped rule foundation has validation, replay, privacy, accessibility, and evidence-language boundaries; directed expansion additionally requires protected general-search evidence, explicit probe-to-expansion economics, resource ceilings, and preregistered evaluation across twenty structural families plus at least three consent-safe authored projects. The full contract is [Rules That Matter 0.7](rules-that-matter-0.7.md).
