...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **8/10** | Durable MCP and CLI campaigns execute named policy-bound windows, while local one-shot portfolio checks now classify sustained work and activate bounded workers with zero pilot replay. The 80-cell gate avoided false worker startups and the 5M Intercept gate retained exact evidence while left-shifting T1 and completion. Multi-run allocation and corpus-proven stopping quality remain (#93). |
...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **8/10** | Automatic portfolio concurrency now shares one state/heap envelope, falls back before unsafe startup, retains partial worker/memory evidence, and leaves hosted jobs on an explicit deployment ceiling. Compact checkpoints also admit the measured 500K Intercept base at 54.1 MiB. Campaign-wide scheduling, externally reconciled cost controls, and broader storage adversaries remain (#93, #152, #156). |
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **6/10** | Exact split-run equivalence remains strong, children preserve a viable base checkpoint byte-for-byte, and streamed gzip now admits the 500K Intercept base at 54.1 MiB with its stable logical ID unchanged. Directed children still restart from the root; frontier reuse, reprioritization, hosted campaigns, specialist partitioning, adversarial storage accounting, and a second public family remain (#66, #86, #93, #156). |
