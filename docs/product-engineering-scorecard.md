...
| **Robust unknown-shape exploration** | The default gives strong broad insurance across early choices, deep suffixes, variables, loops, gates, storylets, randomness, and sparse failures without one family dominating. | **6/10** | DFS, beam, seeded random, shared-frontier experiments, and adaptive rounds are materially stronger than DFS alone. Scale-normalized replay removes the known early-choice regression, but the incomplete corpus still blocks policy promotion (#56, #103). |
