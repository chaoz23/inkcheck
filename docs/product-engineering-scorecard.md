...
| --- | --- | ---: | --- |
| **Actionable, repeatable QA** | Mechanical defects become stable findings with exact replay; fixes can be checked again in CLI, CI, web, or MCP. | **8/10** | Compile/runtime findings, assertions, indexed witnesses, stable private reports, exact replay, and runtime regression pins now support a durable MCP search-fix-recheck loop. The default five-tool MCP profile plus bundled skill measures about 2,738 bootstrap tokens under the preregistered byte estimate; the two-implementation agent gate remains unproven (#67). Assertion/ending pins and broader post-edit campaign proof remain (#66). |
