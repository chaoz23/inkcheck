...
Updated 2026-07-12 against the Inkcheck 0.5.1 release plus unreleased 0.6 research on `main` and draft policy replay work.
...
| **Robust unknown-shape exploration** | The default gives strong broad insurance across early choices, deep suffixes, variables, loops, gates, storylets, randomness, and sparse failures without one family dominating. | **6/10** | DFS, beam, seeded random, shared-frontier experiments, and adaptive rounds are materially stronger than DFS alone. The corpus is incomplete and matched policy replay has exposed a critical sparse-error regression (#56, #103). |
...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **4/10** | Progress, discovery curves, shadow decisions, time/memory guards, and evaluation tooling exist. Policy v1 is not active and fails a matched sparse-error gate; campaigns, deadlines, and concurrency remain (#93-#96, #103). |
...
| **Demonstrated generalization** | Claims are supported across at least 20 consent-safe public stories or structural families, multiple budgets/depths/seeds, and medium/large projects as well as small-source/large-state stories. | **3/10** | Strong adversarial fixtures and useful *The Intercept* studies exist, but *The Intercept* is considered a relatively small source file and the required broad corpus has not been completed (#56). |
...
| **Critical evidence cannot be averaged away** | Runtime errors and assertion violations remain separate, identity-based promotion gates; no aggregate score or terminal count can hide their loss. | **7/10** | Report/evaluator tiers and paired comparisons enforce the principle. The benchmark corpus is not broad enough, and policy v1 currently fails a critical matched window (#56, #103). |
| **Portfolio-marginal value accounting** | Explorers and specialists earn discretionary work only for portfolio-new evidence or explicit intent progress; duplicate rediscovery is visible but not double-paid. | **3/10** | Portfolio marginal ending/knot/error counters exist in the legacy scheduler, but shadow policy reads pass-local curves and can credit duplicates (#105). |
| **Real protected probe floors** | Every active explorer receives an auditable cumulative integer service floor, with bounded debt across windows and no ceiling overrun. | **3/10** | Fractional 8% weights express the intent. Small-window rounding can repeatedly grant zero, so this is not yet a cumulative guarantee (#106). |
...
| **Inspectable decisions** | Every automatic activation, allocation, forecast, contraction, and stop records policy version, factual inputs, uncertainty, reason, and binding constraint in compact human/agent forms. | **7/10** | Shadow decisions and replay logs establish the contract. They are not yet live policy, and compact MCP forecasts/controls remain (#62, #95). |
