...
| **Robust unknown-shape exploration** | The default gives strong broad insurance across early choices, deep suffixes, variables, loops, gates, storylets, randomness, and sparse failures without one family dominating. | **6/10** | DFS, beam, seeded random, shared-frontier experiments, and adaptive rounds are materially stronger than DFS alone. Marginal credit removes the first sparse-error replay loss, but early-choice authored-coverage regressions and an incomplete corpus still block policy promotion (#56, #103, #113). |
...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **4/10** | Progress, discovery curves, shadow decisions, time/memory guards, and evaluation tooling exist. Marginal credit removes one critical replay failure, but policy v1 remains inactive and overcommits after early discoveries (#93-#96, #103, #113). |
...
| **Real protected probe floors** | Every active explorer receives an auditable cumulative integer service floor, with bounded debt across windows and no ceiling overrun. | **5/10** | Policy replay now has an exact cumulative ledger, deterministic debt service, completion release, tiny-window rotation, and 5M accounting tests. It is still research-only; production promotion remains blocked by the early-choice regression (#113). |
