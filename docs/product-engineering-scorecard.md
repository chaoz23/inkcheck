...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **6/10** | Durable MCP result windows now expose partial evidence, exact continuation, compact inspection, cancellation boundaries, and replay before a long base search finishes. Policy v2 adds auditable yield decisions but remains gated off; campaign allocation, deadlines, concurrency, and proven dynamic stopping remain (#93-#96). |
...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **8/10** | Hard state/depth ceilings, shared CLI/MCP/evaluation guards, bounded curves and artifacts, atomic snapshots, hosted limits, partial reports, retained-frontier envelopes, and a 100M combined base/directed MCP campaign ceiling compose safely. Hosted persistence, global concurrency accounting, and full project campaign controls remain (#40, #93, #94). |
