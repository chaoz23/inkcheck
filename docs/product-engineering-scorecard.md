...
| **Anytime value per wall clock** | Users receive useful result windows early; allocation, long-tail probing, deadlines, and stopping adapt to measured yield and resources without hiding uncertainty. | **7/10** | Durable MCP campaigns execute named policy-bound windows across fresh processes, expose preferred marginal yield/spend/provenance, and can stop after three dry windows plus protected long-tail work. The empirical forecast remains one-trajectory evidence with high/medium uncertainty; multi-run allocation, corpus-proven stopping quality, and human UX remain (#93, #94, #96). |
| **Author and agent intent** | Humans and agents can safely declare invariants, goals, deadlines, and resource posture; Inkcheck verifies them mechanically and explains what it could not establish. | **8/10** | Typed assertions, additive/staged goals, exact replay, regression pins, and named MCP campaign modes now cover deadline/resource/value/stop posture with attributable explanations. Directed campaign children, causal goal quality, and the hosted assistant remain (#66, #81, #86, #87, #96). |
...
| **Resource-safe execution** | State, depth, time, heap, frontier, disk, and campaign ceilings compose safely; constrained hosted deployments degrade cleanly. | **8/10** | Hard state/depth ceilings, shared CLI/MCP/evaluation guards, bounded curves and artifacts, atomic snapshots, hosted limits, retained-frontier envelopes, and durable single-frontier campaign state/time/deadline/memory/disk accounting compose safely. Hosted persistence, multi-worker/global concurrency accounting, and cross-run cost controls remain (#40, #93, #94). |
| **Resumable shared search economy** | Diverse frontier/checkpoint states can be retained, compacted, resumed, partitioned, and shared across specialists without reparsing or restarting from the root. | **7/10** | Base shared search now has exact split-run equivalence, private source-bound CLI checkpoints, and durable MCP start/inspect/continue/cancel windows across fresh processes. Reports add bounded storage, finding lookup, exact witness replay, runtime pins, and additive goal probes that preserve the base checkpoint. Directed-frontier resume, reprioritization, hosted campaigns, and specialist partitioning remain (#66, #86, #93). |
| **Inspectable decisions** | Every automatic activation, allocation, forecast, contraction, and stop records policy version, factual inputs, uncertainty, reason, and binding constraint in compact human/agent forms. | **9/10** | MCP campaigns expose a stable policy ID/version, resolved mode and overrides, allocation reason, preferred-yield rate, empirical next-window range, uncertainty, throughput/resources, binding constraint, and report drill-down. Independent-child allocation and externally accounted cost decisions remain absent. |
