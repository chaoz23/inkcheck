...
test("capabilities explicitly reports supported and unavailable features", () => {
  ...
  assert.strictEqual(value.inkcheckVersion, "0.6.0-beta.3");
...
test("Codex agent kit creates synchronized config, CI, ignore rules, and instructions", () => {
  ...
  try {
    ...
    assert.match(fs.readFileSync(path.join(tmp, ".inkcheck", "AGENTS.md"), "utf8"), /Inkcheck 0\.6\.0-beta\.3/);
    assert.match(fs.readFileSync(path.join(tmp, ".github", "workflows", "inkcheck.yml"), "utf8"), /inkcheck@0\.6\.0-beta\.3/);
...
test("versioned JSON reports have stable identities and exact replay instructions", () => {
  ...
  assert.strictEqual(first.inkcheckVersion, "0.6.0-beta.3");
...
test("release version stays synchronized across package and manifests", () => {
  ...
  assert.strictEqual(pkg.version, "0.6.0-beta.3");
