...
test("Codex agent kit creates synchronized config, CI, ignore rules, and instructions", () => {
  ...
  try {
    ...
    assert.match(fs.readFileSync(path.join(tmp, ".inkcheck", "AGENTS.md"), "utf8"), /Inkcheck 0\.4\.1/);
    assert.match(fs.readFileSync(path.join(tmp, ".github", "workflows", "inkcheck.yml"), "utf8"), /inkcheck@0\.4\.1/);
