...
- Experimental `--concurrency N` places persistent passes in bounded worker slots while retaining the same adaptive round allocation, deterministic merge order, aggregate budget heartbeats, and explicit worker-failure evidence. It deliberately defaults to `1`: the adaptive candidate passed one authored 5M gate with exact evidence/schedule parity and a 38.5% wall-clock gain, but broad time-to-finding and resource/cancellation gates remain. See the [concurrency evaluation](docs/concurrency-evaluation.md).
