...
`--concurrency N` is an experimental portfolio worker ceiling from 1 to 16; it defaults to 1. Values above 1 run complementary passes in a rolling, hardware/memory-bounded worker pool with canonical grants and deterministic final merge order. Constrained machines fall back to one worker before spawning. Worker failure retains surviving evidence and reports `truncatedBy.worker`; it never masquerades as a state-budget stop. This first slice uses fixed weighted grants, does not support additive goal-state work, and is not the automatic balanced default: matched promotion evidence must first show wall-clock benefit without critical or severe authored-coverage loss.
...
- Experimental `--concurrency N` can place those passes in bounded worker slots, with deterministic grants and merge order, aggregate budget heartbeats, and explicit worker-failure evidence. It deliberately defaults to `1`: two matched 5M-state *The Intercept* candidates lost 42.4% of terminal identities versus the adaptive sequential baseline, and the rolling candidate was slower. Concurrency will not be promoted until workers retain adaptive round allocation and pass the same evidence and resource gates. See the [concurrency evaluation](docs/concurrency-evaluation.md).
