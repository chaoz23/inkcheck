...
- **`--json`** emits the entire report as a single JSON object (`{ compile, stats, explore }`) on stdout — parse that instead of scraping the pretty output. `explore.passes` includes lifetime telemetry per exploration pass, including a deterministic `discoveryCurve` bounded to 64 samples with separate exact-terminal, fallback visible-outcome, runtime-error, assertion, goal/stage, and knot counts. `discoverySummary` preserves first/latest discovery states and dry-gap facts despite compaction. Portfolio reports also include a run-wide curve in actual interleaved execution order. `explore.schedule` shows how the adaptive rounds spent the budget. Curves report observed yield; they do not claim a plateau is complete coverage or currently change allocation.
