...
inkcheck <story.ink> [--max-depth N] [--max-states N] [--seed N] [--story-seed N] [--search=portfolio|shared|shared-variable] [--concurrency N] [--max-frontier-states N] [--max-frontier-memory MB] [--auto] [--profile] [--next] [--no-min-repro] [--strict] [--save-report] [--save-checkpoint] [--progress=auto|human|ndjson|off] [--human|--json|--markdown]
...
`--concurrency N` is an experimental portfolio worker ceiling from 1 to 16; it defaults to 1. Values above 1 keep complementary passes alive across the existing adaptive rounds in a hardware/memory-bounded worker pool with canonical grants and deterministic final merge order. Constrained machines fall back to one worker before spawning. Worker heap limits and the parent reserve sum to one declared heap envelope; workers publish current heap use through shared memory so the parent can stop all passes cooperatively when aggregate tracked heap binds. Reports expose the plan, aggregate high-water mark, and stop source. Peak RSS can be higher because V8/runtime overhead is not heap. Worker failure retains the latest completed evidence and reports `truncatedBy.worker`; it never masquerades as a state-budget stop. A matched 5M *The Intercept* run preserved exact evidence and schedule digests while reducing wall clock 38.5%, at a 46.8% peak-RSS cost. Hosted deployments separately cap per-job workers and real child cancellation cleans temporary uploads. This authored gate is promising, not broad promotion: full-corpus time-to-finding evidence remains open.
...
- Experimental `--concurrency N` places persistent passes in bounded worker slots while retaining the same adaptive round allocation, deterministic merge order, aggregate budget and heap accounting, cooperative memory stops, and explicit worker-failure evidence. It deliberately defaults to `1`: the adaptive candidate passed one authored 5M gate with exact evidence/schedule parity and a 38.5% wall-clock gain, but broad time-to-finding promotion evidence remains. See the [concurrency evaluation](docs/concurrency-evaluation.md).
- A research-only 1,024-state activation pilot distinguishes cheap/exhaustive, depth-bound, authored-frontier-saturated, and open-frontier workloads. In the checked 80-cell 100K matrix it activated only the two sustained early-choice cells, with no evidence/proof loss; a fresh default-depth 5M *The Intercept* pair retained 4,456 exact terminals while reducing wall time 37.0%. It is not the default: the evaluator restarts after the pilot, explicitly reports 1,024 duplicate state evaluations, and remains production-ineligible until that state can migrate under the original global ceilings.
