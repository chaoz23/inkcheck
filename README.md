...
- **`--json`** emits the entire report as a single JSON object (`{ compile, stats, explore }`) on stdout — parse that instead of scraping the pretty output. `explore.passes` includes lifetime telemetry per exploration pass, including a deterministic `discoveryCurve` bounded to 64 samples with cumulative and marginal exact-terminal, fallback visible-outcome, runtime-error, assertion, goal/stage, knot, and state-novelty counts. `discoverySummary` preserves first/latest discovery states and dry-gap facts despite compaction. Portfolio reports also include a run-wide curve in actual interleaved execution order; wall time stays observational in progress events. `explore.schedule` shows how the adaptive rounds spent the budget. The versioned `shadowDecision` shows what the future anytime policy would recommend and why, including protected per-pass floors and uncertainty. It is observation-only (`applied: false`): it never changes today's search or claims bounded coverage is proof.
